...
export async function fetchSpellLists(rsql: string, page: number, size: number, auth: AuthContextProps): Promise<Page<SpellList>> {
...
export async function createSpellList(dto: CreateSpellListDto, auth: AuthContextProps): Promise<SpellList> {
  ...
  return await callApi(auth, url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dto) });
  
}

export async function updateSpellList(spellListId: string, dto: Partial<UpdateSpellListDto>, auth: AuthContextProps): Promise<SpellList> {
  const url = `${apiSpellsUrl}/spell-lists/${spellListId}`;
  return await callApi(auth, url, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dto) });  
}
